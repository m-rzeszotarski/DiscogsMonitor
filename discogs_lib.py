...
scraper = cloudscraper.create_scraper(delay=2000, captcha={"provider": "auto"})
...
USER_AGENTS = [
    ...
    (
        "Mozilla/5.0 (X11; Linux x86_64; rv:124.0) Gecko/20100101 "
        "Firefox/124.0"
    ),
    (
...
def _build_request_headers(base_headers: dict | None = None) -> dict:
    """Build browser-like headers with light randomization per request."""
    headers = dict(base_headers or {})
    headers.setdefault("User-Agent", random.choice(USER_AGENTS))
    headers.setdefault("Accept-Language", random.choice(ACCEPT_LANGUAGES))
...
def _warmup_session(timeout: int, request_headers: dict) -> None:
    """Prime cookies/challenge state before fetching a marketplace page."""
    try:
        # Tiny random delay to avoid deterministic cadence.
        time.sleep(random.uniform(0.7, 1.8))
        scraper.get("https://www.discogs.com/", timeout=timeout, headers=request_headers)
...
def fetch_listings(
    ...
    for attempt in range(1, max(1, retries) + 1):
        try:
            ...
            _warmup_session(timeout, request_headers)
            time.sleep(random.uniform(0.8, 2.2))
            resp = scraper.get(url, timeout=timeout, headers=request_headers)
            resp.raise_for_status()
            
            # Check Discogs rate limit headers
            remaining = resp.headers.get("X-Discogs-Ratelimit-Remaining")
            if remaining is not None:
                try:
                    remaining = int(remaining)
                    if remaining < 2:
                        reset_time = int(resp.headers.get("X-Discogs-Ratelimit-Reset", 60))
            ...
            if not is_retryable or attempt >= max(1, retries):
                raise
            # Renew scraper session on rate-limit/forbidden to refresh challenge state.
        ...
        # More aggressive backoff: 10s, 30s, 60s
        delay = min(max_retry_delay, 10 * (2 ** (attempt - 1)))
        time.sleep(delay)
...
def _parse_row(row, seen_keys: set) -> Optional[dict]:
    ...
    try:
        ...
        # Determine unique key based on stable seller + price + title
        # This creates a fingerprint that alerts only when price, seller, or title change
        # (NOT item_id or URL - those change between scans for the same listing)
        if seller_id and currency and value and title:
            key = f"seller:{seller_id}|{currency}:{value}|{title}"
        elif currency and value and title:
            key = f"{currency}:{value}|{title}"
        else:
            # Fallback: use item_url if nothing else available
            key = f"url:{item_url}" if item_url else "unknown"
